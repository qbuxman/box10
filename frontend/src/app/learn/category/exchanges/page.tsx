...
import {Gift, GraduationCap, Loader} from "lucide-react"
...
const LearnExchangeCategory = () => {
    ...
    useEffect(() => {
        const getLesson = async () => {
            setIsLoading(true)
            try {
                const response = await fetch("/api/learn/categories/exchanges/lesson")
    ...
    const claimTokenForLesson = async () => {
        ...
        setIsClaim(true)
        await claimToken(address, 10, "exchange-lesson")
            .then((response) => {
                if (response.success) {
        ...
        <div className="max-w-4xl mx-auto p-6 space-y-8">
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center h-64 gap-4">
                    ...
                    <p className="text-lg" style={{ color: '#456882' }}>Chargement en cours...</p>
                </div>
            ) : (
...
export default LearnExchangeCategory
