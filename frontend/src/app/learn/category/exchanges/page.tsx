...
const LearnExchangeCategory = () => {
  ...
  useEffect(() => {
    const getLesson = async () => {
      ...
      await fetch("/api/learn/categories/exchanges/lesson")
        .then(async (response) => {
          setLesson(await response.json())
        })
        .catch(() => {
          toast.error(
            "Une erreur est survenue lors de la récupération du cours."
          )
        })
          ...
          {isConnected && lesson ? (
            <div className="flex justify-center mt-8">
              ...
              >
                {isClaim ? (
                  <>
                    <Loader className="animate-spin" /> Récupération de la
                    récompense...
                  </>
                ) : (
                  <>
