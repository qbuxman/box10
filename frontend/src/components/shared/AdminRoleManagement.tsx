...
const AdminRoleManagement = () => {
  ...
  const addRoleToAddress = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/roles/add-admin", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        toast.success(
          `Rôle d'administrateur ajouté avec succès pour l'adresse ${addressToAddRole}`
        )
  ...
  const removeRoleToAddress = async () => {
    ...
    try {
      ...
      // Appeler l'API route pour retirer le rôle (côté serveur)
      const response = await fetch("/api/roles/remove-admin", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.success) {
        toast.success(
          `Rôle d'administrateur retiré avec succès pour l'adresse ${addressToRemoveRole}`
        )
        toast.info(`Transaction: ${data.txHash}`)
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-lg" style={{ color: "#456882" }}>
          Administrez les permissions DEFAULT_ADMIN_ROLE du smart contract
        </p>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription className="text-white/80">
                Attribuer les droits d'administration à une adresse
              </CardDescription>
            </div>
          </div>
        </CardHeader>
      ...
      <Card className="border">
        <CardHeader className="bg-[#234C6A]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-white text-xl">
                Supprimer le rôle Administrateur
              </CardTitle>
              <CardDescription className="text-white/80">
                Retirer les droits d'administration à une adresse
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="pt-6 space-y-4">
          ...
          <Button
            disabled={!addressToRemoveRole || isLoadingRemove}
