...
function getWalletClientAdmin() {
    console.log('🔍 Debug - Toutes les variables env:', Object.keys(process.env).filter(k => k.includes('ADMIN')))
    console.log('🔍 Debug - ADMIN_PRIVATE_KEY présente:', !!process.env.ADMIN_PRIVATE_KEY)

    const privateKey = process.env.ADMIN_PRIVATE_KEY

    if (!privateKey) {
        console.error('❌ ADMIN_PRIVATE_KEY est undefined')
        throw new Error('ADMIN_PRIVATE_KEY n\'existe pas dans les variables d\'environnement')
    }
